...
- 56 个汉字与 10 个数字，共 66 个字符，每个字符各有白色、灰色一张，共 132 张 PNG。
- 每张 48×48，RGBA 透明背景。
...
选关及两种演示页面使用的 42 张白色字符已同步到 `src/common/image/glyphs/`，通过 `PixelLabel` 组件组合显示。它们沿用相同透明留白规则。其他字符仍保留在素材库中；后续接入时，在 `scripts/sync-glyphs.cjs` 中添加所需映射即可。
